...
$(function () {
    ...
    sortedByFriends = _.sortBy(data, ['friends.length', 'name']);

    var matrixSkills = _.map(sortedByFriends, 'skills', 'friends');
    var arraySkills = _.flattenDeep(matrixSkills);
    var uniqSkills = _.uniq(arraySkills);
    sortedSkills = _.sortBy(uniqSkills);
    
    var arrayFriends = _.flattenDeep(_.map(sortedByFriends, 'friends'));
    console.log(arrayFriends);
    sortedFriends = _.sortBy(_.uniq(_.map(arrayFriends, 'name')));
    


    var template = _.template($('#pattern').html());
    var content = template(sortedByFriends);
    $('body').append(content);
});
